  ...
  multiTimeline.prototype = {
    ...
    init: function() {
      ...
      if (!this._endMoment.isAfter(this._startMoment)) {
        console.error(
          "multi-timline.js: end date has to be a date after start date!"
        );
        return false;
      }
      ...
      this._unitCount =
        this.options.xAxisUnit === "days" ? this._daysCount : this._weeksCount;
      this._unitPercentage =
        this.options.xAxisUnit === "days"
          ? this._percentagePerDay
          : this._percentagePerWeek;
    ...
    addMarks: function() {
      ...
      while (!current.isAfter(end, unit)) {
        ...
        if (
          this._unitCount > 10 &&
          timeUnitCount %
            Math.round(this._unitCount / this.options.maxLabelCount) !==
            0
        ) {
          label = "";
        }
    ...
    addTimeline: function(dataEntry) {
      ...
      if (dataEntry.layer === undefined) {
        useLayer =
          this.options.flatten !== true
            ? this._layerCount
            : this.getNextFreeLayer(
                moment(dataEntry.start),
                moment(dataEntry.end)
              );
      ...
      this._layerUsage.push({
        layer: useLayer,
        start: moment(dataEntry.start),
        end: moment(dataEntry.end)
      });
      ...
      durationIn.seconds = this.getDuration(
        moment(dataEntry.start),
        moment(dataEntry.end)
      );
      ...
      startOffsetIn.seconds = this.getDuration(
        moment(this.options.start),
        moment(dataEntry.start)
      );
      ...
      var styles = {
        ...
        "background-color":
          dataEntry.color !== undefined && !$.isArray(dataEntry.color)
            ? dataEntry.color
            : null
      };
      ...
      if (
        this.options.allResizeable === true ||
        dataEntry.resizeable === true
      ) {
    ...
    getTimelineHtml: function(dataEntry) {
      ...
      // Resize Handler
      html +=
        '<div class="tl-timeline__resizer tl-timeline__resizer-start"></div>';
      html +=
        '<div class="tl-timeline__resizer tl-timeline__resizer-end"></div>';

      // Color Bars
      if ($.isArray(dataEntry.color)) {
        dataEntry.color.forEach(function(color) {
          html +=
            '<div class="tl-timeline__color-bar" style="background-color: ' +
            color +
            '"></div>';
        });
      }
      ...
      if (
        dataEntry.start != this.options.dawn &&
        this.options.markerDateFormat !== false
      ) {
        html +=
          '<div class="tl-timeline__date-marker tl-timeline__date-start">';
    ...
    setWrapperDimensions: function() {
      ...
      this.$element.find(".tl-timeline").each(function() {
        ...
        if (left > 0 && left < 100) {
          ...
          if (layer > that._highestLayer) that._highestLayer = layer;
      ...
      var totalHeight =
        timelineHeight +
        (this._highestLayer + 2) * this.options.timelineSpacing;
      ...
      this.$element
        ...
        .find(".tl-time__unit span")
        .css({ height: totalHeight, top: totalHeight * -1 });
    ...
    addEventHandlers: function() {
      ...
      if (
        this.options.mousewheelPan === true ||
        this.options.mousewheelZoom === true
      ) {
      ...
      that.$element.on("mousedown", ".tl-timeline", function(e) {
        ...
        if ($(this).hasClass("is-resizeable")) {
          if (
            $(e.target).hasClass("tl-timeline__resizer-start") ||
            $(this).hasClass("is-infinite-end")
          ) {
            mode = "w-resize";
          } else if (
            $(e.target).hasClass("tl-timeline__resizer-end") ||
            $(this).hasClass("is-infinite-start")
          ) {
            mode = "e-resize";
          }
        }
        ...
        $mouseMoveTargets.on("mousemove", function(e) {
          ...
          if (totalDelta.y > that.options.timelineSpacing) {
            $drag.css(
              "bottom",
              (currentLayer + 1) * that.options.timelineSpacing + 20 + "px"
            );
            ...
            $drag.css(
              "bottom",
              useForSpacing * that.options.timelineSpacing + 20 + "px"
            );
            $drag.data("tl-layer", currentLayer - 1);
          ...
          that.options.data[
            $drag.data("tl-identifier")
          ].layer = this._layerCount;
          ...
          switch (mode) {
            ...
            case "w-resize":
              ...
              $drag.offset({ left: newLeft }).css("width", dragWidth - delta.x);
              break;
            case "e-resize":
              if (dragWidth + delta.x < 50) return;
          ...
          var percentLeft =
            (100 / parseInt(that.$element.innerWidth())) * pxLeft;
        ...
        $(document).on("mouseup", function() {
          ...
          var percentLeft =
            (100 / parseInt(that.$element.innerWidth())) * oldLeft;
    ...
    updateDates: function($element, startDate) {
      ...
      var percentLeft =
        (100 / parseInt(this.$element.innerWidth())) *
        (pxLeft + $element.outerWidth());

      var endDate = $element.hasClass("is-infinite-end")
        ? moment(this.options.infinity)
        : this.percentToDate(percentLeft);

      // Update data objects
      this.options.data[
        $element.data("tl-identifier")
      ].start = startDate.format("YYYY-MM-DD HH:mm:ss");
      this.options.data[$element.data("tl-identifier")].end = endDate.format(
        "YYYY-MM-DD HH:mm:ss"
      );
      ...
      if (this.options.markerDateFormat !== false) {
        $element
          .find(".tl-timeline__date-start")
          .html(startDate.format(this.options.markerDateFormat));
      }
      if (this.options.markerDateFormat !== false) {
        $element
          .find(".tl-timeline__date-end")
          .html(endDate.format(this.options.markerDateFormat));
      }
    },
    ...
    percentToDate: function(percent) {
      ...
      var add = parseInt(
        percent * unitPercent * secondsPer[this.options.xAxisUnit]
      );
      ...
      date.set(
        "minute",
        Math.round(minutes / this.options.gridPrecision) *
          this.options.gridPrecision
      );
      date.set("second", 0);
    ...
    zoomOut: function(levels) {
      ...
      this.options.start = moment(this.options.start)
        .subtract(levels * this.options.zoomStep, "days")
        .format("YYYY-MM-DD");
      this.options.end = moment(this.options.end)
        .add(levels * this.options.zoomStep, "days")
        .format("YYYY-MM-DD");
    ...
    zoomIn: function(levels) {
      ...
      var newStart = moment(this.options.start).add(
        levels * this.options.zoomStep,
        "days"
      );
      var newEnd = moment(this.options.end).subtract(
        levels * this.options.zoomStep,
        "days"
      );
    ...
    goRight: function() {
      ...
      this.options.start = moment(this.options.start)
        .add(jump, this.options.xAxisUnit)
        .format("YYYY-MM-DD");
      this.options.end = moment(this.options.end)
        .add(jump, this.options.xAxisUnit)
        .format("YYYY-MM-DD");
    ...
    goLeft: function() {
      var jump = Math.ceil(this._unitCount / 12);
      this.options.start = moment(this.options.start)
        .subtract(jump, this.options.xAxisUnit)
        .format("YYYY-MM-DD");
      this.options.end = moment(this.options.end)
        .subtract(jump, this.options.xAxisUnit)
        .format("YYYY-MM-DD");
  ...
  $.fn[pluginName] = function(options) {
    return this.each(function() {
      if (!$.data(this, pluginName)) {
        $.data(this, pluginName, new multiTimeline(this, options));
      }
      return this;
    });
  };
