const Routific = {
    import(calculator, db, ids, routes)
    {
    ...
    getTimeTravelsOnRoute(index){
        ...
        .map(function(node,i){
            if(i == routific.routes[index].length - 1)
                return -1
    ...
    getRoutificOnRoute(index){
        ...
        let timeStartOnDepot = this.calculator.getThoiDiemDiNenBatDau(
            timeWindowsOnRoute,
            serviceTimesOnRoute, 
            timeTravelsOnRoute.filter(
                function(x,i){
                    if(i == timeTravelsOnRoute.length - 1)
                        return false
                    return true
                })
        )

        let timeEndOnDepot = timeStartOnDepot + 
